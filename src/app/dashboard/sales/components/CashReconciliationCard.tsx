...
export default function CashReconciliationCard({
  ...
  const expected = (openingCash ?? 0) + todayCash;
  ...
  const onSubmit = async (data: CashFormValues) => {
    const cashVariance = data.closingCash - (data.openingCash + todayCash);
    try {
  ...
  return (
    <div className='rounded-2xl border border-amber-100 shadow-sm bg-amber-50/40 p-5'>
      {/* Header */}
      <div className='flex items-start justify-between mb-4'>
        <div>
          <h2 className='text-sm font-semibold tracking-widest uppercase text-amber-500 flex items-center gap-2'>
      ...
      {showForm && (
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
          ...
          {/* Live preview */}
          {hasPreview && (
            <div className='grid grid-cols-3 gap-3 p-4 bg-white border border-amber-100 rounded-xl text-xs'>
              {[
                {
                  label: "Expected",
                ...
                <div key={label} className='text-center'>
                  ...
                  <p className={`font-bold tabular-nums text-sm ${color}`}>
                    {value}
                  </p>
                </div>
              ))}
